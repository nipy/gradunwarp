...
Note that a core component of `gradient_unwarp.py` (`unwarp_resample.py`) uses a `subprocess` call to the FSL tools `fslval` and `fslorient`. So FSL must be [installed]:https://fsl.fmrib.ox.ac.uk/fsl/fslwiki/FslInstallation/Linux and its configuration file correctly [sourced]:https://fsl.fmrib.ox.ac.uk/fsl/fslwiki/FslInstallation/ShellSetup (i.e., `FSLDIR` and `FSLOUTPUTTYPE` must be defined appropriately in your environment, and `${FSLDIR}/bin` must be in your `PATH` and contain `fslval`, `fslorient`, and `fslhd`).
